...
from model import SmoothL1_Classifier_Node, SmoothL1_Regressor_Node
...
#        1. Server transmit the hyperparameters to all clients
        ...
        for round_num in range(self.n_rounds):
            ...
            for client in clients:
                ...
                print(f"  Client {client.client_id}: acc: {result['acc']:.4f}%, sparsity: {result['sparsity']:.2f}%")
            
            # Print round results
            avg_acc = np.mean([r['acc'] for r in round_results])
            avg_spar = np.mean([r['sparsity'] for r in round_results])
            
            print(f"  Average accuracy: {avg_acc:.4f}, Average sparsity: {avg_spar:.2f}%")
...
def main():
    ...
    clients = initialize_clients(n_clients)
    ...
    # Test final model performance on all clients
    # print("\nFinal model performance on all clients:")
    # all_results = []
    # for client in clients:
    #     result = client.evaluate()
    #     client.model.reset()
    ...
    # print(f"  Average accuracy: {avg_acc:.4f}, Average sparsity: {avg_sparsity:.2f}%")
