# this exp is for 
# testing different batch size for the performance of ESN+SmoothL1 with gradient optimization
...
if __name__ == "__main__":
    ...
    # ================================================================================================
    s_X_tr_sample = s_X_tr[0]
    train_batches_Y_sample = train_batches_Y[0]
    s_X_te_sample = s_X_te[10]
    test_batches_Y_sample = test_batches_Y[10]


    readout = Ridge(ridge=1e-2)
    readout.fit(s_X_tr_sample, train_batches_Y_sample)

    pred_train = readout.predict(s_X_tr_sample)
    pred_test = readout.predict(s_X_te_sample)

    plt.plot(pred_train, label="train")
    plt.plot(train_batches_Y_sample, label="true")
    plt.show()
    plt.savefig("train_pred.png")

    plt.plot(pred_test, label="test")
    plt.plot(test_batches_Y_sample, label="true")
    plt.show()
