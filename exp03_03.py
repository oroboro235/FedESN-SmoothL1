...
def initialize_clients(n_clients: int, no_cross: bool = False, same_test:bool = True, n_sample_train_per_client: int = 500, n_sample_test_per_client: int = 500):
    # load the data
    ...
    # 
    from data_loader import read_data_har
    X_train, Y_train, X_test, Y_test = read_data_har()

    # turned Y_train to one-hot encoding
    Y_train = np.eye(6)[Y_train.astype(int)]
    Y_test = np.eye(6)[Y_test.astype(int)]
    ...
    if no_cross:
        ...
        for i in range(n_clients):
            ...
            idx = np.random.choice(len(X_test), n_sample_test_per_client, replace=False)
    ...
    for i in range(n_clients):
        if same_test:
            client = Client(i, (X_train_split[i], Y_train_split[i], X_test_split[0], Y_test_split[0]), seed=random_seeds[i])
        else:
