...
from funcs import mse_sl1_fval, mse_sl1_grad, ce_sl1_fval, ce_sl1_grad, ce_none_fval, ce_none_grad, ce_l2_fval, ce_l2_grad
...
class Reg_Node(TrainableNode):
    ...
    # calculation part
    def fit(self, x: NodeInput, y: Optional[NodeInput] = None, warmup: int = 0, isFL: bool = False) -> np.ndarray:
        # 
        ...
        # -------------------------------------------------------------------
        # Smooth L1 regularization with L1General (no PyTorch)
        # -------------------------------------------------------------------
        if self.reg_type == "sl1" and not isFL:
            from functools import partial
            from L1General_python.lossFuncs import SquaredError
            from L1General_python.L1General import L1GeneralUnconstrainedApx
            ...
            # Build augmented design matrix if bias is learned
            if self.fit_bias:
                X_aug = np.hstack([x, np.ones((x.shape[0], 1))])
                lambda_vec_base = self.reg_param * np.ones(self.input_dim + 1)
                lambda_vec_base[-1] = 0.0   # no regularization on bias
            else:
                X_aug = x
                lambda_vec_base = self.reg_param * np.ones(self.input_dim)

            for i in range(self.output_dim):
                if self.fit_bias:
                    W0 = np.zeros((self.input_dim + 1, 1))
                else:
                    W0 = np.zeros((self.input_dim, 1))

                funcObj = partial(SquaredError, X=X_aug, y=y[:, i].reshape(-1,1))
                options = {"verbose": 0, "mode": 0, "progTol": 1e-12}
                lambda_vec = lambda_vec_base.copy()
                W_opt, _ = L1GeneralUnconstrainedApx(funcObj, W0, lambda_vec, options)

                if self.fit_bias:
                    _Wout[:, i] = W_opt[:-1].reshape(-1)
                    _bias[i] = W_opt[-1][0]
                else:
                    _Wout[:, i] = W_opt.reshape(-1)
        ...
        # -------------------------------------------------------------------
        # Smooth L1 regularization using PyTorch (isFL=True)
        # -------------------------------------------------------------------
            ...
            if self.fit_bias:
                _bias = torch.tensor(self.bias, requires_grad=True)
                params = [_Wout, _bias]
            else:
                _bias = None
                params = [_Wout]

            optimizer = SGD(params, lr=self.learning_rate)
            ...
            for e in range(self.epochs):
                ...
                # Compute gradient w.r.t. weights (using centered targets)
                if self.fit_bias:
                    y_centered = y_t.detach().numpy() - _bias.detach().numpy()
                else:
                    y_centered = y_t.detach().numpy()

                w_grad = mse_sl1_grad(_Wout.detach().numpy(),
                                      x_t.detach().numpy(),
                                      y_centered,
                                      self.alpha,
                                      self.reg_param)
                ...
                if self.fit_bias:
                    residuals = y_t.detach().numpy() - pred.detach().numpy()
                    bias_grad = -np.sum(residuals, axis=0)   # shape (output_dim,)
                    _bias.grad = torch.tensor(bias_grad)

                optimizer.step()
                self.update_alpha(e)   # update smoothing parameter

                # Copy learned parameters back to the node
                self.Wout = _Wout.detach().numpy()
                if self.fit_bias:
                    self.bias = _bias.detach().numpy()

                # Apply sparsity threshold to weights (not bias)
                self.Wout[np.abs(self.Wout) < self.thres] = 0.0

        # -------------------------------------------------------------------
        # Standard L1 / no regularization (scikit-learn Lasso)
        # -------------------------------------------------------------------
        elif self.reg_type in ["none", "l1"]:
            ...
            for i in range(self.output_dim):
                if self.reg_type == "l1":
                    rgr = Lasso(alpha=self.reg_param, fit_intercept=self.fit_bias, max_iter=1000)
                else:   # "none"
                    rgr = Lasso(alpha=0.0, fit_intercept=self.fit_bias, max_iter=1000)

                rgr.fit(x, y[:, i])
                _Wout[:, i] = rgr.coef_.reshape(-1).copy()
                if self.fit_bias:
                    _bias[i] = rgr.intercept_
...
class Clr_Node(TrainableNode):
    """
    Docstring for Classifier_Node
    """
    # Regularization Params
    reg_param: float
    # bias term flag
    ...
    def fit(self, x: NodeInput, y: Optional[NodeInput] = None, warmup: int = 0) -> np.ndarray:
        ...
        if self.reg_type == "sl1":
            fval_func = ce_sl1_fval
            grad_func = ce_sl1_grad
        elif self.reg_type == "none":
            fval_func = ce_none_fval
            grad_func = ce_none_grad
        elif self.reg_type == "l2":
            fval_func = ce_l2_fval
            grad_func = ce_l2_grad
        else:
        ...
        for e in range(self.epochs):
            ...
            optimizer.zero_grad()
            if self.reg_type == "sl1":
                loss = fval_func(_Wout.detach().numpy(), last_x, y, self.alpha, self.reg_param)
            else:
                loss = fval_func(_Wout.detach().numpy(), last_x, y, self.reg_param)
            # calculate the gradient and update the weights
            if self.reg_type == "sl1":
                w_grad = grad_func(_Wout.detach().numpy(), last_x, y, self.alpha, self.reg_param)
            else:
                w_grad = grad_func(_Wout.detach().numpy(), last_x, y, self.reg_param)
...
class Clr_Node_old(TrainableNode):
    """
    Docstring for Classifier_Node
    """
    # Regularization Params
    ...
    def fit(self, x: NodeInput, y: Optional[NodeInput] = None, warmup: int = 0) -> np.ndarray:
        ...
        if self.reg_type == "sl1" and isFL:
            import torch
            from torch.optim import SGD, Adams, RMSprop, Adagrad

            _Wout = torch.tensor(self.Wout, requires_grad=True)
            # optimizer = SGD([_Wout], lr=self.learning_rate)
            optimizer = Adam([_Wout], lr=self.learning_rate)
            # optimizer = RMSprop([_Wout], lr=self.learning_rate)
            # optimizer = Adagrad([_Wout], lr=self.learning_rate)
        ...
        elif self.reg_type == "sl1" and not isFL:
        ...
        elif self.reg_type in ["l1", "l2", "none"]:
            from sklearn.linear_model import LogisticRegression
            from sklearn.multiclass import OneVsRestClassifier

            _Wout = np.zeros((self.input_dim, self.output_dim))
            if self.reg_type == "l1":
                lgr = OneVsRestClassifier(LogisticRegression(penalty='l1', 
                                     C=1.0/self.reg_param, 
                                     solver='liblinear',
                                     fit_intercept=self.fit_bias))
            elif self.reg_type == "l2":
                lgr = OneVsRestClassifier(LogisticRegression(penalty='l2', 
                                     C=1.0/self.reg_param, 
                                     solver='liblinear',
                                     fit_intercept=self.fit_bias))
            else:
            ...
            if self.reg_type == "none":
                _Wout = lgr.coef_.T.copy()
            else:
                for i, est in enumerate(lgr.estimators_):
                    _Wout[:, i] = est.coef_.reshape(-1).copy()
...
if __name__ == '__main__':
    ...
    # # calculate mse
    # mse = ((model.run(X_test1)[-1] - y_test1) ** 2).mean()
    # print(f"MSE: {mse:.4f}")
